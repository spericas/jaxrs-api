...
injected into a class field using `@Inject` or directly into a method
parameter. Access to the `Application` subclass instance
...
An instance of `UriInfo` can be injected into a class field or a
resource method parameter as shown below.
`UriInfo` provides both static and dynamic, per-request information,
about the components of a request URI. E.g. the following would return
the names of any query parameters in a request:
...
An instance of `HttpHeaders` can be injected into a class field using
`@Inject` or directly as a method parameter as shown below. `HttpHeaders` provides
...
into a class field using `@Inject` or directly into a method parameter
as shown below.
...
`SecurityContext` can be injected into a class field using `@Inject`
or directly as resource method parameter as shown below. The methods of
...
into a class field using `@Inject` or directly as a resource method parameter.
